#!/usr/bin/env python3
"""
Experiment: UNet + EfficientNet-B5

Same configuration as baseline (B4) but with B5 encoder.
...
def main():
    print("="*80)
    print("EXPERIMENT: UNet + EfficientNet-B5")
    print("="*80)
    print()
    ...
    config.encoder_name = "efficientnet-b5"  # <<< ONLY CHANGE
    ...
    config.checkpoint_dir = os.path.join(config.output_dir, "checkpoints", "efficientnet_b5")
    ...
    print(f"This (B5) CV:     {mean_dice:.4f}")
    ...
    print("="*80)
    print(f"Baseline (B4) Test:  0.6428")
    print(f"This (B5) Test:      {test_results['mean_dice']:.4f}")
    diff_test = test_results['mean_dice'] - 0.6428
    print(f"Difference:          {diff_test:+.4f}")
    
    if test_results['mean_dice'] > 0.6428:
        print("\n✅ B5 OUTPERFORMS B4!")
