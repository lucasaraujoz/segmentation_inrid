#!/usr/bin/env python3
"""
Experiment: UNet + EfficientNet-B2

Same configuration as baseline (B4) but with B2 encoder.
...
def main():
    print("="*80)
    print("EXPERIMENT: UNet + EfficientNet-B2")
    print("="*80)
    print()
    ...
    config.encoder_name = "efficientnet-b2"  # <<< ONLY CHANGE
    ...
    config.checkpoint_dir = os.path.join(config.output_dir, "checkpoints", "efficientnet_b2")
    ...
    print(f"This (B2) CV:     {mean_dice:.4f}")
    ...
    print(f"This (B2) Test:      {test_results['mean_dice']:.4f}")
