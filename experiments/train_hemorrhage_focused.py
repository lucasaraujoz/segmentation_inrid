#!/usr/bin/env python3
"""
Experiment 12: Hemorrhage-Focused Training

SIMPLEST modification: Just increase hemorrhage class weight.

Current baseline:
- Exudates weight: 1.0 (Dice 0.70)
- Hemorrhages weight: 1.3 (Dice 0.58)

New strategy:
- Exudates weight: 1.0 (already good)
- Hemorrhages weight: 2.0 (force model to care more)

NO architecture change. NO new channels. Just reweight loss.
"""
...
def main():
    print("="*80)
    print("EXPERIMENT 12: Hemorrhage-Focused Loss Weighting")
    print("="*80)
    print()
    print("Baseline class weights: [1.0, 1.3]")
    print("New weights: [1.0, 2.0] ← force model to focus on hemorrhages")
    print()
    print("Current performance:")
    print("  Exudates: 0.70 (already saturated)")
    print("  Hemorrhages: 0.58 (BOTTLENECK)")
    print()
    print("Target: Hemorrhage Dice 0.61+ → Overall 0.65+")
    print("="*80)
    print()
    ...
    # ONLY CHANGE: Increase hemorrhage weight
    config.class_weights = [1.0, 2.0]  # was [1.0, 1.3]
    
    config.checkpoint_dir = os.path.join(config.output_dir, "checkpoints", "hemorrhage_focused")
    ...
    print(f"Exudates:  {test_results['dice_exudates']:.4f} (baseline: 0.70)")
    print(f"Hemorrhages: {test_results['dice_haemorrhages']:.4f} (baseline: 0.58)")
    
    print("\n" + "="*80)
    print("COMPARISON")
    print("="*80)
    print(f"Baseline (weights 1.0/1.3):  0.6428")
    print(f"This (weights 1.0/2.0):      {test_results['mean_dice']:.4f}")
    ...
    if test_results['mean_dice'] >= 0.650:
        ...
        print(f"\n❌ Hemorrhage overweighting hurt overall performance")
    
    # Check if hemorrhage improved even if overall dropped
    if test_results['dice_haemorrhages'] > 0.58:
        print(f"✓ Hemorrhage Dice improved: 0.58 → {test_results['dice_haemorrhages']:.4f}")
