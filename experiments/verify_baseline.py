#!/usr/bin/env python3
"""
Re-run baseline to confirm Dice 0.6442.

Uses frozen splits for exact reproducibility.
"""
...
def main():
    print("="*80)
    print("BASELINE VERIFICATION: UNet + EfficientNet-B4")
    print("="*80)
    print()
    print("Expected Dice: 0.6442")
    print("Using frozen CV splits for exact reproducibility")
    print()
    print("="*80)
    print()
    ...
    # Override checkpoint dir for this verification
    config.checkpoint_dir = os.path.join(config.output_dir, "checkpoints", "baseline_verify")
    ...
    for fold_idx in range(5):
        ...
        model_path = os.path.join(config.checkpoint_dir, f"best_model_fold_{fold_idx + 1}.pth")
    ...
    print("="*80)
    print(f"Expected CV:  0.5444")
    print(f"Obtained CV:  {mean_dice:.4f}")
    
    diff_percent = ((mean_dice - 0.5444) / 0.5444) * 100
    print(f"Difference: {diff_percent:+.2f}%")
    
    if abs(diff_percent) < 2.0:
        print("\n✅ CV RESULTS CONSISTENT: Within 2% of expected")
    else:
        print(f"\n⚠️  CV DIFFERS by {abs(diff_percent):.2f}%")
    ...
    obtained_test = test_results['dice']
    
    print(f"Expected Test (Ensemble+TTA):  {expected_test:.4f}")
    print(f"Obtained Test (Ensemble+TTA):  {obtained_test:.4f}")
