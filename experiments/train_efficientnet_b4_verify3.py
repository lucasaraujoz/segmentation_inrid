#!/usr/bin/env python3
"""
Re-verify Baseline B4 (third run)

Confirm Test Dice ~0.6448
"""
...
def main():
    print("="*80)
    print("BASELINE RE-VERIFICATION: UNet + EfficientNet-B4 (Run #3)")
    print("="*80)
    print()
    print("Previous runs: 0.6448, 0.6448")
    print("Confirming baseline is still stable...")
    print()
    print("="*80)
    print()
    ...
    config.checkpoint_dir = os.path.join(config.output_dir, "checkpoints", "efficientnet_b4_verify3")
    ...
    print(f"\nExpected CV: 0.5444")
    print(f"This run CV:  {mean_dice:.4f}")
    ...
    print("="*80)
    print(f"Previous runs (B4):  0.6448, 0.6448")
    print(f"This run (B4):       {test_results['mean_dice']:.4f}")
    
    expected = 0.6448
    diff = test_results['mean_dice'] - expected
    print(f"Difference:          {diff:+.4f}")
    
    if abs(diff) < 0.005:
        print("\n✅ BASELINE STABLE - Within 0.5% of previous runs")
    elif abs(diff) < 0.01:
        print("\n≈ BASELINE CONSISTENT - Within 1% of previous runs")
    else:
        print(f"\n⚠️  BASELINE DIFFERS by {abs(diff):.4f}")
